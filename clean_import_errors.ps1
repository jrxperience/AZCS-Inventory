param(
    ...
    [string]$OutputCsv = "C:\Codex\AZCS Inventory\square_ready\CURRENT\UPLOAD_INVENTORY_CLEANED.csv"
)
...
$removeRows = New-Object System.Collections.Generic.HashSet[int]
$groupMap = @{}
foreach ($correction in $corrections) {
    $groupKey = $correction.GroupRows
    if (-not $groupMap.ContainsKey($groupKey)) {
        $groupMap[$groupKey] = New-Object System.Collections.Generic.List[object]
    }
    $groupMap[$groupKey].Add($correction) | Out-Null
    if ($correction.IssueType -eq "Archive State Mismatch" -and $correction.RecommendationStatus -eq "Archived Duplicate Candidate") {
        [void]$removeRows.Add([int]$correction.RowNumber)
    }
}

$fieldMap = @{
    "Customer-facing Name Mismatch" = "Customer-facing Name"
    "SEO Title Mismatch" = "SEO Title"
    "Item Name Mismatch" = "Item Name"
    "Social Media Link Title Mismatch" = "Social Media Link Title"
}

foreach ($groupKey in $groupMap.Keys) {
    $entries = [object[]]$groupMap[$groupKey].ToArray()
    $issueType = $entries[0].IssueType
    if (-not $fieldMap.ContainsKey($issueType)) {
        continue
    }
    $fieldName = $fieldMap[$issueType]
    $primary = $entries | Where-Object { $_.RecommendationStatus -eq "Keep Active Candidate" } | Select-Object -First 1
    if (-not $primary) {
        $primary = $entries | Select-Object -First 1
    }
    $recordIndex = [int]$primary.RowNumber - 7
    if ($recordIndex -lt 0 -or $recordIndex -ge $records.Count) {
        continue
    }
    $primaryValue = [string]$records[$recordIndex].$fieldName
    foreach ($entry in $entries) {
        $targetIndex = [int]$entry.RowNumber - 7
        if ($targetIndex -lt 0 -or $targetIndex -ge $records.Count) {
            continue
        }
        $records[$targetIndex].$fieldName = $primaryValue
    }
}
...
for ($index = 0; $index -lt $records.Count; $index++) {
    $rowNumber = $index + 7
    if (-not $removeRows.Contains($rowNumber)) {
        $cleaned.Add($records[$index]) | Out-Null
    }
}

Write-TemplatedCsv -Path $OutputCsv -Prelude $source.Prelude -Records ([object[]]$cleaned.ToArray())
...
$summary = @(
    ...
    "Rows removed: $($removeRows.Count)",
    "Rows kept: $($cleaned.Count)",
