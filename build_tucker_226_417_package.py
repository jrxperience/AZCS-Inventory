...
SOURCE_TEMPLATE_CSV = SQUARE_READY_DIR / "TUCKER_225" / "TUCKER_225_IMPORT.csv"
FULL_TUCKER_CSV = BASE_DIR / "quarantine" / "2026-03-20_cleanup" / "square_ready" / "TUCKER" / "TUCKER_ONLY_INVENTORY.csv"
IMAGE_DATABASE_CSV = BASE_DIR / "outputs" / "inventory_database_with_images.csv"
PACKAGE_DIR = SQUARE_READY_DIR / "TUCKER_226_417"
PACKAGE_CSV = PACKAGE_DIR / "TUCKER_226_417_IMPORT.csv"
PACKAGE_IMAGE_DIR = PACKAGE_DIR / "IMAGES"
PACKAGE_MAP_CSV = PACKAGE_DIR / "TUCKER_226_417_IMAGE_MAP.csv"
...
def main() -> None:
    if not FULL_TUCKER_CSV.exists():
        raise FileNotFoundError(f"Missing {FULL_TUCKER_CSV}")
    ...
    source_rows = read_csv_rows(FULL_TUCKER_CSV)
    ...
    start_index = header_index + START_ROW
    end_index = header_index + END_ROW + 1
    selected_rows = source_rows[start_index:end_index]
    ...
    for position, row in enumerate(selected_rows, start=START_ROW):
    ...
    PACKAGE_README.write_text(
        "\n".join(
            [
                ...
                f"TUCKER_226_417_IMPORT.csv = Tucker rows {START_ROW} through {END_ROW} in Square import-template format.",
                "IMAGES/ = corresponding local images copied for those rows.",
                "TUCKER_226_417_IMAGE_MAP.csv = row-by-row image checklist.",
    ...
    print(f"Tucker 226-417 package: {PACKAGE_DIR}")
